...
Barcode::IATA2of5 - Create pattern for IATA 2 of 5 (also known as
"2 of 5" or "Industrial 2 of 5" barcodes

=head1 SYNOPSIS

    use Barcode::IATA2of5;
    
    my $bc = new Barcode::IATA2of5;
	 my $text = '123459';
...
Barcode::IATA2of5 creates the patterns that you need to display
IATA 2 of 5 barcodes.  This symbology was created by the International Air
Transport Association for use in the air cargo industry.  It's basically
a Standard 2 of 5 with different start/stop codes, and the length of the code
is 17 digits.


The pattern returned is a string of 1's and 0's,
...
Do not use this encoding for a new application outside the air cargo industry.
...
Create a new Barcode::IATA2of5 object. The different options that can
be set are:
...
Example:
	 ...
	 $bc = new Barcode::IATA2of5(addcheck => 1);

    Create a new IATA 2/5 barcode object that will automatically add check digits.
...
Untested.  However, it barely differs from Standard 2 of 5, which works.
The author is unsure if the check digit routine is the same.
